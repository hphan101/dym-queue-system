...
function doPost(e) {
  ...
  try {
    ...
    if (!sheet) {
      console.error(
        "Configured branch sheet was not found: " + branchConfig.sheetName,
      );
    ...
    // Chỉ xếp hàng tại đoạn cấp STT và ghi Sheet. Validation, honeypot và tìm tab
    // đều chạy song song trước đó, giúp giảm thời gian chờ khi nhiều khách submit cùng lúc.
    try {
      lock.waitLock(5000);
    } catch (error) {
      return createJsonResponse({
...
function migrateBranchSheetsToNewLayout() {
  ...
  for (var i = 0; i < branchCodes.length; i++) {
    ...
    if (
      oldHeaders[0] !== "Số thứ tự" ||
      oldHeaders[1] !== "Thời gian submit" ||
      oldHeaders[2] !== "Họ tên" ||
      oldHeaders[3] !== "Số điện thoại"
    ) {
      throw new Error(
        "Sheet " +
          config.sheetName +
          " không có cấu trúc cũ như dự kiến. Không tự chuyển dữ liệu.",
      );
    }

    var lastRow = sheet.getLastRow();
    var oldRows =
      lastRow > 1 ? sheet.getRange(2, 1, lastRow - 1, 12).getValues() : [];
    ...
    sheet.clearContents();
    sheet
      .getRange(1, 1, newRows.length, REGISTRATION_HEADERS.length)
      .setValues(newRows);
    sheet.getRange(1, 1, 1, REGISTRATION_HEADERS.length).setFontWeight("bold");
  }
}
...
function getRegistrationCooldownSeconds() {
  var configuredValue = PropertiesService.getScriptProperties().getProperty(
...
function createRegistrationFingerprint(phoneNumber, cccd, branch) {
  ...
  var normalizedCccd = String(cccd)
    .replace(/[^A-Za-z0-9]/g, "")
    .toUpperCase();
...
function isValidPayloadShape(data) {
  ...
  return (
    ...
    (!Object.prototype.hasOwnProperty.call(data, "honeypot") ||
...
function isValidRegistrationData(data) {
  return (
    isValidText(data.fullName, 2, 100) &&
